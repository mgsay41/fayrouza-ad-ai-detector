...
function AppContent() {
  ...
  const getDecisionLabel = useCallback((decision: 'Approve' | 'Review' | 'Reject') => {
    switch (decision) {
      case 'Approve':
  ...
  }, [state.data, state.isLoading, toast, t, format, getDecisionLabel])
  ...
  const handleSubmit = async (data: Parameters<typeof submit>[0]) => {
    const result = await submit(data, false)
        ...
        <Dialog open={showClearDialog} onOpenChange={setShowClearDialog}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>{t.history.clearAllDescription}</DialogDescription>
            </DialogHeader>
            <DialogFooter>
