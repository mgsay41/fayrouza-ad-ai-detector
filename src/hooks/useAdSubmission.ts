...
import { submitAdToWebhook, mockSubmitAdToWebhook, ApiError } from '../lib/api'
  ...
  const submit = useCallback(
    async (
      ...
      try {
        ...
        const shouldUseMock =
          useMock || !import.meta.env.VITE_N8N_WEBHOOK_URL

        const { response, responseTime } = shouldUseMock
          ? await mockSubmitAdToWebhook(data)
          : await submitAdToWebhook(data)
